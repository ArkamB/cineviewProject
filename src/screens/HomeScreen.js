...
import TrendingMovies from "../components/TrendingMovie/index"; // Corrected import path
...
export default function HomeScreen() {
  ...
  const { isLoading: isTrendingLoading, error: trendingError } = useQuery({
  ...
  const { isLoading: isGenresLoading, error: genresError } = useQuery({
    queryKey: ["genres"],
    queryFn: fetchGenres,
    onSuccess: (data) => {
  ...
  if (trendingError || topRatedError || popularError || upcomingError || genresError) {
    return (
      <View className="flex-1 justify-center items-center">
        <Text className="text-red-500">Error loading data. Please try again later.</Text>
      </View>
    );
  }
